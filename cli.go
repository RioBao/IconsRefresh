...
func run(cfg config) error {
	...
	if result.ShellNotify != nil {
		fmt.Printf("shell_notify: assoc_changed=%t env_broadcast=%t timeout_ms=%d warning=%q\n",
			result.ShellNotify.AssocChangedSent,
			result.ShellNotify.EnvironmentBroadcast,
			result.ShellNotify.EnvironmentTimeoutMS,
			result.ShellNotify.Warning,
		)
	}
